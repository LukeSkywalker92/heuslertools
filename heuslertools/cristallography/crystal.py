...
class Crystal():
    ...
    def show_crystal_boarder(self, ax, size, color, lw):
        ax.plot([0, size], [0, 0], [0, 0], f'{color}-', lw=lw)
        ax.plot([0, size], [size, size], [0, 0], f'{color}-', lw=lw)
        ax.plot([0, size], [0, 0], [size, size], f'{color}-', lw=lw)
        ax.plot([0, size], [size, size], [size, size], f'{color}-', lw=lw)

        ax.plot([0, 0], [0, size], [0, 0], f'{color}-', lw=lw)
        ax.plot([size, size], [0, size], [0, 0], f'{color}-', lw=lw)
        ax.plot([0, 0], [0, size], [size, size], f'{color}-', lw=lw)
        ax.plot([size, size], [0, size], [size, size], f'{color}-', lw=lw)

        ax.plot([0, 0], [0, 0], [0, size], f'{color}-', lw=lw)
        ax.plot([size, size], [0, 0], [0, size], f'{color}-', lw=lw)
        ax.plot([0, 0], [size, size], [0, size], f'{color}-', lw=lw)
        ax.plot([size, size], [size, size], [0, size], f'{color}-', lw=lw)
